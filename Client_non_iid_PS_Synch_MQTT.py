...
# Client counterpart in FL with parameter server Completely Synch (NO TFF)
...
if __name__ == '__main__':
    ...
    if NUM_PARTS_DATASET != 1:    
        train_dataset_preprocess = dataset.create_tf_dataset_for_client_train_non_iid(client_, client_, 0)
    else:
        train_dataset_preprocess = dataset.create_tf_dataset_for_client_train_non_iid(client_, client_)                
    valid_dataset_preprocess = dataset.create_tf_dataset_for_client_valid_non_iid(client_, client_)
    ...
    # Recover round
    messages = subscribe.simple(client_weights_topic + '/{}/#'.format(client_), qos=2, msg_count=MODEL_SNIPPETS, retained=True, hostname=broker_address,
    ...
    for round_ in range(START_ROUND, NUM_ROUNDS):
        ...
        # First round common initialization
        if round_ == START_ROUND: 
            retain_flag = True # Non-Blocking
        else:
            retain_flag = False # Blocking

        # The client are waiting the global model from the server
        # print('Waiting global weights')
        messages = subscribe.simple(server_weights_topic + '/#', qos=2, msg_count=MODEL_SNIPPETS, retained=retain_flag, hostname=broker_address,
                            port=MQTT_port, client_id="Client" + str(client_), keepalive=10, will=None, auth=AUTH_, tls=TLS_,
                            protocol=mqtt.MQTTv311)
...
##############################################################################################################
        ...
        if NUM_PARTS_DATASET != 1:
            train_dataset_preprocess = dataset.create_tf_dataset_for_client_train_non_iid(client_, client_, round_)
...
##############################################################################################################
        ...
        for snippet in range(MODEL_SNIPPETS):
            ...
            messages.append({'topic':client_weights_topic + '/{}/{}'.format(client_, snippet), 'payload': payload, 'qos': 2, 'retain': False})
